...
class Sync_postgre_model extends CI_Model{
    ...
    public function get_all_vendor_data()
    {
        ...
        $this->postgre->select("vendor_id, vendor_name, contact_name, contact_phone_no, contact_email, address_street, login_id, password, dir_name, dir_pos");
        ...
        $query = $this->postgre->get('vnd_header');
        return $query->result();
    }
...
'%GM Departemen%'",'left')
        ...
        */
        $this->postgre->select("a.dept_id, a.dept_name, a.dep_code , b.pos_id , b.pos_name, c.fullname as complete_name")
        ->from('adm_dept a')
    ...
    public function get_all_data_lelang()
    {
        ...
        $this->postgre->select("a.subject_work
        , b.ptm_dept_name
        , a.vendor_name
        , a.start_date
        , a.end_date
        ...
        {
            $time = $last_sync->row()->created_at;
            $this->postgre->where('created_date >=', $time);
        }
        $this->postgre->from('ctr_contract_header as a')
    ...
    {
        $this->postgre->select("*");
        $query = $this->postgre->get('adm_pos');
    ...
    public function get_all_data_users()
    {
        ...
        */
        $this->postgre->select('a.id, a.complete_name, c.pos_id , c.pos_name, a.user_name , b.email, a."password" , c.dept_id ')
        ->from('adm_user a')
    ...
    public function get_all_data_kontrak()
    {
        ...
        $this->postgre->select("nama_spk_full
        , kode_spk
        , updated_date
        , kddivisi
        , divisiname
        ...
        $query = $this->postgre->get('project_info');
        return $query->result();
    }
    ...
    {
        $this->postgre->select("*");
        $query = $this->postgre->get('ctr_ammend_header');

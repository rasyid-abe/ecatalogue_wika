...
class Order_menu_model extends CI_Model
{
	...
	public function getAllById($where = array()){
		$this->db->select("order_menu.*")->from("order_menu");
		$this->db->where($where); 
		$this->db->where("order_menu.is_deleted",0); 
	...
	public function update($data,$where){
		$this->db->update('order_menu', $data, $where);
    ...
    {
    	$this->db->select("order_menu.*")->from("order_menu"); 
       	$this->db->limit($limit,$start)->order_menu_by($col,$dir) ;
    	if(!empty($search)){
    		$this->db->group_start();

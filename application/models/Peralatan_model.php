...
a.product_id ")
	...
	public function getperalatan($where = array())
	{
...
AND CASE WHEN d.amandemen_id IS NULL
	...
	public function findperalatan($where = array())
	{
		$this->db->select("peralatan.*");
		$this->db->from('peralatan');
		$this->db->where($where);
	...
	{
		$this->db->insert('peralatan_download', $data);
		return $this->db->insert_id();
	}
	...
	function getAllBy($limit, $start, $search, $col, $dir, $where = array())
	{
		...
		j2.name as jenis_name2,
	  project_new.name as project_name,
	  users.username as user_name
      
    ");
		$this->db->from('peralatan');
		$this->db->join('jenis', 'jenis.id = peralatan.id_jenis', 'left');
		$this->db->join('jenis j2', 'j2.id = jenis.parent_id', 'left');
		$this->db->join('users', 'users.id = peralatan.created_by', 'left');
		$this->db->join('project_new', 'project_new.id = peralatan.id_proyek', 'left');
	...
	function getAllBy2($limit, $start, $search, $col, $dir, $where = array())
	{
		$this->db->select("peralatan_download.*,peralatan.*,
		jenis.name as jenis_name,
		project_new.name as project_name,
		users.username as user_name,
		groups.name as divisi_name,
		peralatan_download.created_at as tgl_download
      
    ");
		$this->db->from('peralatan_download');
		$this->db->join('peralatan', 'peralatan.id = peralatan_download.id_peralatan', 'left');
		$this->db->join('jenis', 'jenis.id = peralatan.id_jenis', 'left');
		$this->db->join('project_new', 'project_new.id = peralatan.id_proyek', 'left');
		$this->db->join('users', 'users.id = peralatan_download.created_by', 'left');
		$this->db->join('groups', 'groups.id = peralatan.id_divisi', 'left');
		$this->db->limit($limit, $start)->order_by($col, $dir);
		if (!empty($search)) {
			$this->db->group_start();
			foreach ($search as $key => $value) {
	...
	function getCountAllBy($limit, $start, $search, $order, $dir, $where = array())
	{
		...
		$this->db->select("COUNT(*) as Total")->from("peralatan");
		$this->db->join('jenis', 'jenis.id = peralatan.id_jenis', 'left');
		$this->db->join('project_new', 'project_new.id = peralatan.id_proyek', 'left');
		if (!empty($search)) {
			$this->db->group_start();
	...
	function getCountAllBy2($limit, $start, $search, $order, $dir, $where = array())
	{

		$this->db->select("COUNT(*) as Total")->from("peralatan_download");
		$this->db->join('peralatan', 'peralatan.id = peralatan_download.id_peralatan', 'left');
		$this->db->join('jenis', 'jenis.id = peralatan.id_jenis', 'left');
		$this->db->join('project_new', 'project_new.id = peralatan.id_proyek', 'left');
		if (!empty($search)) {
	...
	public function get_dropdown($where = [])
    {
        ...
        $query = $this->db->select('name, id')->where($where)->get('peralatan_status')->result();
        foreach($query as $v)
        {

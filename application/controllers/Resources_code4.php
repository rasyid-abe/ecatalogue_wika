...
class Resources_code4 extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->data['is_can_read']) {
            $this->data['title'] = 'Level 4';
            $this->data['content']  = 'admin/resources_code/list_code4';
            $this->data['data'] = $this->db->get_where('resources_code', ['level' => 4])->result_array();
        } else {
            redirect('restrict');
        }
    ...
    public function get_level1_edit()
    {
        ...
        $c = $this->db->get_where('resources_code', ['code' => $l2['parent_code']])->row_array();
        $d = $this->db->get_where('resources_code', ['code' => $c['parent_code']])->row_array();
        $result = [
            'code' => $d['code'],
            'name' => $d['name'],
    ...
    public function create()
    {
        if ($this->data['is_can_create']) {
            if (!empty($_POST)) {
                ...
                $inc = $this->Resources_code_model->increment($level3, 4);
                $code = substr($level3, 0, 3) . $inc['data'] . '00';
                ...
                if ($inc['data'] == 'err') {
                    ...
                    $arr_data = [
                        ...
                        'level' => 4,
                        'sts_matgis' => $this->db->get_where('resources_code', ['code' => $level3])->row('sts_matgis'),
                    ];
                    ...
                    $log = [
                        'code' => $code,
                        'activity' => 'Insert kode level 4 (kode perkiraan = ' . $code . '-' . $name . ')',
    ...
    public function update()
    {
        if ($this->data['is_can_edit']) {
            if (!empty($_POST)) {
                ...
                $log = [
                    'code' => $code_log,
                    'activity' => 'Update kode level 4 (kode perkiraan = ' . $code_log . '-' . $name . ')',
                    'created_by' => $this->data['users']->id
                ];
                $this->db->insert('resources_code_logs', $log);
        ...
        if ($this->data['is_can_delete']) {
            ...
            foreach ($ids as $key => $value) {
                ...
                if ($data['code'] == '') {
                    ...
                    $inc = $this->Resources_code_model->increment($parent, 4);
                    $code = substr($parent, 0, 3) . $inc['data'] . '00';
                    ...
                    $log = [
                        'code' => $code,
                        'activity' => 'Delete kode level 4 (kode perkiraan = ' . $code . '-' . $data['name'] . ')',
                        'created_by' => $this->data['users']->id
                    ];
                    $this->db->insert('resources_code_logs', $log);
                } else {
                    ...
                    if ($child > 0) {
                        ...
                        $log = [
                            'code' => $data['code'],
                            'activity' => 'Delete kode level 4 (' . $data['code'] . '-' . $data['name'] . ')',
                            'created_by' => $this->data['users']->id
                        ];
                        $this->db->insert('resources_code_logs', $log);
    ...
    private function approve_data($ids, $sts)
    {
        if ($this->data['is_can_approval']) {
            ...
            foreach ($ids as $key => $value) {
                ...
                if ($d['status'] != $sts) {
                    $inc = $this->Resources_code_model->increment($d['parent_code'], 4);
                    $code = substr($d['parent_code'], 0, 3) . $inc['data'] . '00';
                    ...
                    $log = [
                        'code' => $code,
                        'activity' => $log_desc . ' kode level 4 (' . $log_d . $code . '-' . $data['name'] . ')',
                        'created_by' => $this->data['users']->id
                    ];
                    $this->db->insert('resources_code_logs', $log);
                }
            ...
            //     $log = [
            //         'code' => $data['code'],
            //         'activity' => $log_desc . ' kode level 4 (' . $data['code'] . '-' . $data['name'] . ')',
            //         'created_by' => $this->data['users']->id
            //     ];
            //     $this->db->insert('resources_code_logs', $log);

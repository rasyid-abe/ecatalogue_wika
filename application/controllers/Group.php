...
class Group extends Admin_Controller {
	...
	public function create()
	{
        ...
        $this->form_validation->set_rules('general_manager',"General Manager", 'trim|required');
	...
	{
        //redirect('group');
		$this->form_validation->set_rules('name', "Name Is Required", 'trim');
        $this->form_validation->set_rules('general_manager',"General Manager", 'trim');
        $this->form_validation->set_rules('departemen_code2',"Departemen Code", 'trim');
		...
		if ($this->form_validation->run() === TRUE)
		{
		 	$group_id = $id;
			$data = array(
				'departemen_code2' => $this->input->post('departemen_code2'),
			);
            //die(var_dump($this->input->post()));
            if ($_FILES['ttd']['name'])
            {
                $location_path = "./image_upload/ttd_gm/";

                $filename = pathinfo($_FILES['ttd']['name']);
                $extension = $filename['extension'];

                $uploaded = uploadFile("ttd", $location_path, 'ttd_gm', "." . $extension);
                if($uploaded['status'] === TRUE)
                {
                    $data['ttd'] = $uploaded['message'];
                }
            }
            //die();
		...
		else
		{
			if(!empty($_POST)){
                ...
                $this->data['general_manager'] =   (!empty($data))?$data[0]->general_manager:"";
				$this->data['description'] =   (!empty($data))?$data[0]->description:"";
                $this->data['departemen_code2'] =   (!empty($data))?$data[0]->departemen_code2:"";
        ...
        if(!empty($this->input->post('search')['value'])){
        	$search_value = $this->input->post('search')['value'];
           	$search = array(
                ...
                "groups.general_manager"=>$search_value,
                "groups.departemen_code2"=>$search_value,
           	);
            ...
            foreach ($datas as $key=>$data)
            {
            	...
            	if($this->data['is_can_delete']){
	            	if($data->is_deleted == 0){
	        			...
	        			$delete_url = "<a href='#'
	        				url='".base_url()."group/destroy/".$data->id."/".$data->is_deleted."'
	        				class='btn btn-sm btn-success  white delete'
	        				 >Aktifkan
	        				</a>";
	        		}
        		}
                ...
                $nestedData['general_manager'] = $data->general_manager;
           		$nestedData['description'] = substr(strip_tags($data->description),0,50);
                $nestedData['departemen_code2'] = $data->departemen_code2;

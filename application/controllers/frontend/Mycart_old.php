...
class Mycart extends Admin_Controller {
    ...
    public function index()
    {
        ...
        else
        {
            ...
            $whereLocation = ['is_deleted' => 0, 'is_prov' => 0];
            $this->data['location'] = $this->Location_model->get_dropdown($whereLocation);
    ...
    public function pay()
    {
        ...
        */
        ...
        for($i = 0; $i < count($product); $i++)
        {
            ...
            $whereTransport = [
                'transportasi.id' => $this->input->post('transportasi_id')[$this->input->post('vendor_id')[$i]][$location_id[$i]][$this->input->post('category_id')[$i]],
            ];
            $dataTransport = $this->Transportasi_model->getHargaByLastAmandemen($whereTransport);
            if ($dataTransport !== FALSE)
            {
                $biayaTransport = $dataTransport->price;
            ...
            if( ! array_key_exists($vendor_id[$i].'_'.$arr_payment_method_id[$i].'_'.$location_id[$i] . '_' . $is_matgis[$i] . '_' . $kontrak_id, $order))
            {
                ...
                $total_price[$index] = 0 ;
                // order_transportasi
                if ($this->input->post('transportasi_id')[$this->input->post('vendor_id')[$i]][$location_id[$i]][$this->input->post('category_id')[$i]])
                {

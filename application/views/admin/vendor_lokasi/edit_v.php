      ...
      <div class="box-body">
        ...
        <table class="table table-bordered table-responsive-md table-striped">
            ...
            <tr>
              <td class="aduh">
                <select class="select-td col-sm-12" name="vendor_id" required>
                  ...
                  <?php foreach ($dataVendor as $value) : ?>
                    <option value="<?php echo $value->id; ?>" <?php if ($vendor == $value->id) echo 'selected="selected"'; ?>><?php echo $value->name; ?></option>
                  <?php endforeach; ?>
                </select>
              </td>
            </tr>
          ...
          <tbody id="tbody-row">
            ...
            <tr>
              <?php
              foreach ($dataLokasi as $values) { ?>
                <td class="aduh">
                  <b>Lokasi</b>
                  <select class="select-td col-sm-12" name="location_id[]" required>
                    <option value="" disabled selected>Pilih Lokasi</option>
                    <?php foreach ($location as $value) :

                    ?>
                      <option value="<?php echo $value->location_id; ?>" <?php if ($values->wilayah_id == $value->location_id) echo 'selected="selected"'; ?>><?php echo $value->full_name; ?></option>
                    <?php endforeach; ?>
                  </select>
                  <div class="box-body">
                    <div class="form-group row">
                      ...
                      <div class="col-sm-9">
                        <input type="name" class="form-control" placeholder="Alamat" name="alamat" value="<?php echo $values->alamat; ?>">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="inputEmail3" class="col-sm-3 control-label">Latitude</label>
                      <div class="col-sm-9">
                        <input type="name" class="form-control" placeholder="Latitude" name="latitude" id="latitude" value="<?php echo $values->latitude; ?>">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="inputEmail3" class="col-sm-3 control-label">Longitude</label>
                      <div class="col-sm-9">
                        <input type="name" class="form-control" placeholder="Longitude" name="longitude" id="longitude" value="<?php echo $values->longitude; ?>">
                      </div>
                    </div>
                    <div class="form-group row">
...
<script type="template" id="template-row-product">
  ...
  <tr>
        <td class="aduh">
          <select class="select-td required col-sm-12" name="location_id[]" required>
              <option value="" disabled selected>Pilih Lokasi</option>
        <?php foreach ($location as $value) : ?>
